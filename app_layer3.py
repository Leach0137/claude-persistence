...
socketio = SocketIO(
    app,
    cors_allowed_origins="*",  # Configure this more strictly in production
    async_mode='eventlet',
    ping_timeout=60,
    ping_interval=25
)
...
instance_metadata = {}   # Track instance info
...
def update_memory():
    ...
    # Broadcast update to all connected instances in the same room
...
def handle_connect():
    ...
    emit('connected', {
        ...
        'timestamp': datetime.now(timezone.utc).isoformat(),
        'message': 'Connected to Claude Persistence Server'
    })
...
def handle_disconnect():
    ...
    if sid in active_connections:
        ...
        # Notify other instances in the same room
...
def handle_register_instance(data):
    """Register a Claude instance and join its room"""
    instance_id = data.get('instance_id', 'default')
    ...
    print(f'Instance registered: {instance_id} (session: {sid})')
    ...
    # Notify other instances in the room
...
Ready for real-time Claude instance communication.
""")
    
    # Use eventlet server for production
    socketio.run(
        app,
        host='0.0.0.0',
        port=port,
